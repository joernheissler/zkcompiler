...
/** \brief tuple of witness, commitment and homomorphism **/
struct witness {
    /** \brief witness (secret) **/
    const char *w;
    /** \brief commitment (public) **/
    const char *x;
    /** \brief homomorphism **/
    const char *h;
};

void rnd(Parser *p, struct witness *w, unsigned n, unsigned long k)
{
    /* choose random witnesses */
    p->getVar(w[k].w)->random();
    
    /* calculate public value */
    p->map(w[k].x, w[k].h, w[k].w);
    ...
    /* witness for k=0,1 is the same, but we need to calculate *both* public values */
    if(k <= 1) p->map(w[1 - k].x, w[1 - k].h, w[1 - k].w);
}
...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        struct witness wx[] = { {"w0a","x0a","phi0a"}, {"w0a","x0b","phi0b"}, {"w0c","x0c","phi0c"}, {"w0d","x0d","phi0d"},
                                {"w1a","x1a","phi1a"}, {"w1a","x1b","phi1b"}, {"w1c","x1c","phi1c"},
                                {"w2a","x2a","phi1a"}, {"w2a","x2b","phi1b"}, {"w2c","x2c","phi1c"},
                                {0, 0, 0} };
        bnz_t r;
        bnz_init(r);
        ...
        bnz_set_ui(r, 4);
        Random::range0(r, r);
        rnd(prover, wx + 0, 4, bnz_get_ui(r));
        
        bnz_set_ui(r, 3);
        Random::range0(r, r);
        rnd(prover, wx + 4, 3, bnz_get_ui(r));
        
        bnz_set_ui(r, 3);
        Random::range0(r, r);
        rnd(prover, wx + 7, 3, bnz_get_ui(r));
